...
/**
 * Created by afon on 2016/12/6.
 */

public class NumberListWheelView extends View {
    public static final String TAG = "NumberListWheelView";
...
//    private GestureDetectorCompat mGestureDetectorCompat;
    private GestureMoveDetectorCompat mGestureDetectorCompat;
    ...
    public NumberListWheelView(Context context) {
        super(context);
        init(null);
    }

    public NumberListWheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public NumberListWheelView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        mBottomSpace = density * 6;
        mBottomSpace = WheelView.dpTopx(getContext(), 14);
...
//        mTopSpace = mCursorSize + density * 2;
        ...
        mGestureDetectorCompat = new GestureMoveDetectorCompat(getContext(), onGestureDetectListener);
...
//        int result = (int) (mBottomSpace + mTopSpace * 2 + mCenterTextSize);
        int result = (int) (mBottomSpace + mTopSpace + mCenterTextSize);
        switch (measureMode) {
            case MeasureSpec.EXACTLY:
...
//        canvas.drawPath(mCenterIndicatorPath, mMarkPaint);
        ...
        float startX = mMaxOverScrollDistance + getScrollX();
        canvas.drawLine(startX, 0, startX, mTopSpace, mMarkPaint);
        canvas.drawLine(startX, mHeight - mTopSpace, startX, mHeight, mMarkPaint);

        int start = mCenterIndex - mViewScopeSize - 1;
        int end = mCenterIndex + mViewScopeSize + 2;
...
//                    canvas.drawLine(ox, mTopSpace + smallMarkShrinkY, ox, mTopSpace + markHeight - smallMarkShrinkY, mMarkPaint);
            ...
            if (mMarkCount > 0 && i >= 0 && i < mMarkCount) {
                ...
                float drawY = mTopSpace + mCenterTextSize;
                if (mCenterIndex == i) {
                    mMarkTextPaint.setColor(mHighlightColor);
                    mMarkTextPaint.setTextSize(mCenterTextSize);
                    if (!TextUtils.isEmpty(mAdditionCenterMark)) {
                        ...
                        canvas.drawText(temp, 0, temp.length(), x - off, drawY, mMarkTextPaint);
                        mMarkTextPaint.setTextSize(mNormalTextSize);
                        canvas.drawText(mAdditionCenterMark, x + tsize / 2f, drawY, mMarkTextPaint);
                    } else {
                        canvas.drawText(temp, 0, temp.length(), x, drawY, mMarkTextPaint);
                    }
                } else {
    ...
    private GestureMoveDetectorCompat.OnGestureDetectListener onGestureDetectListener = new GestureMoveDetectorCompat.OnGestureDetectListener() {
        @Override
        public void onDown(MotionEvent e, float x, float y) {
            if (!mScroller.isFinished()) {
        ...
        @Override
        public void onHorizontalMove(MotionEvent e, float x, float y) {
            if (null != getParent()) {
                getParent().requestDisallowInterceptTouchEvent(true);
            }
        }
        ...
        @Override
        public void onVerticalMove(MotionEvent e, float x, float y) {
            if (null != getParent()) {
        ...
        @Override
        public void onClick(MotionEvent e, float x, float y) {
    ...
    public interface OnWheelItemSelectedListener {
        void onWheelItemChanged(NumberListWheelView wheelView, int position);

        void onWheelItemSelected(NumberListWheelView wheelView, int position);
    }
    ...
    static class SavedState extends BaseSavedState {
        ...
        public String toString() {
            return "NumberListWheelView.SavedState{"
